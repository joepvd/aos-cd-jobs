    ...
    stage("Sync to mirror") {
        if (params.LOCATION.startsWith("rcm-guest")) {
            ...
            commonlib.shell(
                ...
                set -euxo pipefail
		cp --help
                cp --verbose --recursive --preserve=all --dereference --copy-contents ${source_path} ${local_dest_dir}
        ...
        if (params.SET_LATEST) {
            commonlib.shell(
                ...
                set -euxo pipefail
                rm -rf ${latest_dir}
                cp --verbose --recursive --preserve=all --dereference --copy-contents ${local_dest_dir} ${latest_dir}
		ls -l ${latest_dir}
                """
            )
