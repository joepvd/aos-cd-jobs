    ...
    slackChannel.task("Public release prep for: ${FROM_RELEASE_TAG}${ params.DRY_RUN ? ' (DRY RUN)' : ''}") {
        ...
        sshagent(['aos-cd-test']) {
            ...
            stage("validate RHSAs") {
                ...
                release.getAdvisoryIds().each {
                    res = commonlib.shell(
                        script: "${buildlib.ELLIOTT_BIN} --group ${group} verify-rhsa ${it}",
                        returnAll: true,
                    )
                    if (res.returnStatus != 0) {
                        slacklib.to(version).say("Review of CVE situation required for advisory <https://errata.devel.redhat.com/advisory/${it}|${it}>. Report:\n```${res.stdout}\n```")
                    }
                }
            }
        }
